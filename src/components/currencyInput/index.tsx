...
export const CurrencyInput = (props: {
  ...
  const renderPopularTokens = tokens.map((item) => {
    return (
      <Option
        ...
        style={{color: "FFF", whiteSpace: "nowrap", textOverflow: "ellapsis", overflow: "hidden" }}
      >
        <div
          key={item.mintAddress}
          style={{ display: "flex", alignItems: "center", color: "FFF", whiteSpace: "nowrap", textOverflow: "ellapsis", overflow: "hidden" }}
        >
          <TokenIcon style={{marginRight: "5px"}} mintAddress={item.mintAddress} />
          {item.tokenSymbol}
        </div>
      </Option>
    );
  ...
  return (
      <div>
      <div className="ccy-input-header" style={{ color: "#A7A0BD"}}>
      <div className="ccy-input-header-left" style={{ color: "#A7A0BD"}}>{props.title}</div>

      </div>
      <div className="ccy-input-header" style={{ color: "#A7A0BD", padding: "0px 10px 5px 7px" }}>
        <NumericInput
          value={props.amount}
          onChange={(val: any) => {
        ...
        />
        <div
          className="ccy-input-header-balance"
          onClick={(e) =>
        ...
        >
          ...
          <div>
        <div className="ccy-input ccy-input-header-right" style={{ display: "flex", }}>
          <Select
            className="select"
            size="large"
            style={{  width: "auto", marginLeft: "-30px", backgroundColor: "transparent", }}
            placeholder="CCY"
            value={props.mint}
